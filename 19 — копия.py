...
async def exit_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    exit_message = await query.message.reply_text("Вы вышли из бота. Чтобы начать снова, отправьте команду /start.")
    context.user_data['exit_message_id'] = exit_message.message_id
...
async def handle_start_again(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Удаление сообщений: /start и "Вы вышли из бота..."
    start_message_id = context.user_data.get('start_message_id')
    exit_message_id = context.user_data.get('exit_message_id')
    ...
    # Отправляем новое сообщение или выполняем другие действия
    await query.message.reply_text("Вы снова в боте!")
###
#async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Сохраняем сообщение пользователя с командой /start
    context.user_data['start_message_id'] = update.message.message_id

    # Отправляем сообщение с кнопкой
    keyboard = [[InlineKeyboardButton("Старт", callback_data="start_again")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    # Отправляем сообщение о выходе из бота
    exit_message = await update.message.reply_text(
        "Вы вышли из бота. Чтобы начать снова, нажмите 'Старт'.",
        reply_markup=reply_markup
    )

    # Сохраняем id сообщения о выходе
    context.user_data['exit_message_id'] = exit_message.message_id
"""
###
